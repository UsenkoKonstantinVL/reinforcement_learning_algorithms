...
class Agent:
    ...
    def _define_model(self, input_placeholder):
        ...
        with tf.variable_scope('Value'):
            self.v = tf.layers.dense(fc2, self.num_actions, activation=tf.nn.relu)

        with tf.variable_scope('Advantage'):
            self.adv = tf.layers.dense(fc2, 1, activation=tf.nn.relu)

        with tf.variable_scope('Q'):
            q = self.v + (self.adv - tf.reduce_mean(self.adv, axis=1, keep_dims=True))
